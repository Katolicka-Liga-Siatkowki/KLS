...
export async function POST(request: Request) {
  const config = env as unknown as { RESEND_API_KEY?: string; CONTACT_EMAIL?: string };
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { authorization: `Bearer ${config.RESEND_API_KEY}`, "content-type": "application/json" }, body: JSON.stringify({
    from: "KLS <onboarding@resend.dev>", to: [config.CONTACT_EMAIL || "katolickaligasiatkowki@gmail.com"], reply_to: parsed.data.email,
